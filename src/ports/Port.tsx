...
import { canConnectPortDataTypes } from './portCompatibility'
...
export function Port({ shapeId, portId }: { shapeId: TLShapeId; portId: PortId }) {
	...
	const isEligible = useValue(
		...
		() => {
			...
			// `eligiblePorts.terminal` is the terminal being dragged, which this port shares, so
			// this port is the one at that end and the recorded type is the other end's.
			if (
				eligiblePorts.dataType &&
				!(port.terminal === 'end'
					? canConnectPortDataTypes(eligiblePorts.dataType, port.dataType)
					: canConnectPortDataTypes(port.dataType, eligiblePorts.dataType))
			)
				return false
			return true
		},
	...
	return (
		<div
			className={classNames(
				...
				isCollectionDataType(port.dataType) && 'Port_collection',
