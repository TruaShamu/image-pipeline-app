...
import { isCollectionDataType, PORT_TYPE_COLORS, PortDataType } from '../constants'
import {
...
import { canConnectPortDataTypes, findFirstCompatiblePort } from '../ports/portCompatibility'
...
export class ConnectionShapeUtil extends ShapeUtil<ConnectionShape> {
	...
	onHandleDrag(connection: ConnectionShape, { handle }: TLHandleDragInfo<ConnectionShape>) {
		...
		// Check type compatibility. `draggingTerminal` says which end is being moved, and so
		// which of the two types is the source: dragging the 'end' means the fixed opposite
		// end is the output feeding this target.
		const isTypeIncompatible =
			target &&
			dragDataType &&
			!(draggingTerminal === 'end'
				? canConnectPortDataTypes(dragDataType, target.port.dataType)
				: canConnectPortDataTypes(target.port.dataType, dragDataType))
...
function ConnectionShapeComponent({ connection }: { connection: ConnectionShape }) {
	...
	const connectionType = useValue(
		'connectionType',
		() => {
			const bindings = getConnectionBindings(editor, connection.id)
			if (!bindings.start) return null
			return getPortDataType(editor, bindings.start.toId, bindings.start.props.portId)
		},
		[connection.id, editor]
	)
	const connectionColor = connectionType ? PORT_TYPE_COLORS[connectionType] : null
	const isCollection = connectionType ? isCollectionDataType(connectionType) : false
	...
	const path = getConnectionPath(start, end)
	return (
		<SVGContainer
			className={classNames(
				'ConnectionShape',
				isInactive && 'ConnectionShape_inactive',
				isCollection && 'ConnectionShape_collection'
			)}
		>
			{/*
			 * A collection is drawn as two parallel rails: a wide coloured stroke with a narrow
			 * background-coloured stroke down its middle. Splitting the line this way follows the
			 * curve exactly at any angle, unlike an offset copy of the path.
			 */}
			{isCollection && (
				<path
					className="ConnectionShape-rail"
					d={path}
					style={connectionColor ? { stroke: connectionColor } : undefined}
				/>
			)}
			<path
				className={isCollection ? 'ConnectionShape-split' : undefined}
				d={path}
				style={connectionColor && !isCollection ? { stroke: connectionColor } : undefined}
			/>
		</SVGContainer>
	)
}
