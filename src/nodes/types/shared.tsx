...
import { extensionForMimeType, withMatchingExtension } from '../../tools/imageFilenames'
...
export abstract class NodeDefinition<Node extends { type: string }> {
	...
	getTitle(_node: Node): string {
		return this.heading ?? this.title
	}

	getIcon(_node: Node): TLUiIconJsx {
		return this.icon
	}
...
/**
 * A small button that downloads the given image URL (data URL, R2 path, or
 * external URL) to the user's machine.
 *
 * The extension comes from the fetched bytes, not from a fixed default. A GIF saved as `.png`
 * opens in the wrong application and shows as a still frame, which reads as "the GIF block
 * produced an image".
 */
export function NodeImageDownloadButton({ url, filename }: { url: string; filename?: string }) {
	const onClick = useCallback(
		async (e: React.MouseEvent) => {
			...
			try {
				...
				a.href = objectUrl
				a.download = filename
					? withMatchingExtension(filename, blob.type)
					: `image-${Date.now()}.${extensionForMimeType(blob.type) ?? 'png'}`
...
export function coerceToNumber(value: PipelineValue, fallback = 0): number {
	...
	const n = parseFloat(typeof value === 'string' ? value : String(value))
